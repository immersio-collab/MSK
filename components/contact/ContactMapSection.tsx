...
export const ContactMapSection: React.FC = () => {
  const infos = [
    ...
    {
      icon: <Bus className="w-6 h-6 text-msk-forest-500" />,
      title: "Accès",
      ...
      <div className="container mx-auto px-4 max-w-6xl">
        <div className="flex flex-col lg:flex-row gap-12 items-center">
          ...
          <div className="lg:w-1/3 space-y-8">
            <h2 className="text-3xl font-bold text-msk-forest-950 mb-8">
              Venir au centre
            </h2>
            <div className="space-y-6">
              {infos.map((info, idx) => (
                <div key={idx} className="flex items-start gap-4">
                  <div className="w-12 h-12 rounded-xl bg-white border border-msk-forest-100 flex items-center justify-center shrink-0 shadow-sm">
                    {info.icon}
                  </div>
                  <div>
                    <h3 className="text-lg font-bold text-msk-forest-950">{info.title}</h3>
                    <p className="text-msk-forest-700">{info.desc}</p>
                  </div>
                </div>
              ))}
            </div>
          ...
          {/* Map Embed */}
          <div className="lg:w-2/3 w-full h-[400px] md:h-[500px] bg-white rounded-3xl p-2 border border-msk-forest-100 shadow-xl overflow-hidden relative">
